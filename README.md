...
- Proficiency-based course & stage progression (CEFR/JLPT) / 숙련도 기반 코스 및 단계 진도 (CEFR/JLPT)
...
- Verb conjugation tables (per course: Spanish 22×7, German 20×6, Japanese 17×10) / 동사 활용표
- Grammar rule pages (course-specific rules) / 문법 규칙
...
- Accent-aware input helpers (per course: á,é,í,ó,ú,ñ for Spanish; ä,ö,ü,ß for German) / 악센트 입력 도우미
- Score tracking with streak counter / 점수 및 연속 정답 추적
- Practice hub with drill + flashcard + quiz links / 연습 허브

### Audio & Tooltips / 음성 & 툴팁
- TTS pronunciation (Web Speech API, dynamic locale: es-ES, de-DE, ja-JP) / TTS 발음 재생
- Vocabulary tooltips in lesson text (click-to-popup) / 레슨 내 어휘 툴팁

### Dashboard / 대시보드
- Proficiency stage progress tracking (CEFR/JLPT) / 숙련도 단계별 진도 추적 (CEFR/JLPT)
...
This viewer reads content from the companion [language-hub](https://github.com/eunsu-park/language-hub) repository (Spanish, German, Japanese courses). By default, it expects `language-hub` as a sibling directory:

```
parent/
├── language-hub/          # Content repository (Spanish, German, Japanese)
